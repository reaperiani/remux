...
def mux_audio_video():
    ...
    video_no_audio = ffmpeg.input(input_video).video
    input_audio_stream = ffmpeg.input(input_audio).audio
    muxed_video = ffmpeg.output(video_no_audio, input_audio_stream, output_file, vcodec='copy', acodec='copy')

    muxed_video.run(overwrite_output=True)
